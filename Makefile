...
BIN=cat echo sleep true false bridge tty sync shell test_crt0 id clear pwd uname yes wc kill [ mkdir expr printf
ARCH=$(shell uname -m)
START=start.S
...
kill: kill.c
	@case $(ARCH) in \
		armv8l) \
			...
			;; \
		x86_64) \
			$(CROSS) $(CFL) -nostdlib -static -fno-stack-protector $(START) -o $@ $<; \
			;; \
		*) \
			printf "unsupported architecture $(ARCH)\n"; \
			exit 1; \
...
$(filter-out false true sync shell test_crt0 id clear touch pwd uname yes cat echo tty sleep wc kill mkdir [ expr printf,$(BIN)): %: %.c
	@case $(ARCH) in \
		armv8l) \
			$(CC) -s -static -o $@ $<; \
			;; \
