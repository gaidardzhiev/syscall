...
BIN=cat echo sleep true false bridge tty sync shell test_crt0 id clear pwd uname
ARCH=$(shell uname -m)
...
$(filter-out false true sync shell test_crt0 id clear touch pwd uname,$(BIN)): %: %.c
	@case $(ARCH) in \
		armv8l) \
			$(CC) -s -static -o $@ $<; \
			;; \
