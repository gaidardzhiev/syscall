...
BIN=cat echo sleep true false bridge tty sync shell test_crt0 id clear pwd uname yes wc kill [ mkdir expr
ARCH=$(shell uname -m)
START=start.S
...
$(filter-out false true sync shell test_crt0 id clear touch pwd uname yes cat echo tty sleep wc kill mkdir [ expr,$(BIN)): %: %.c
	@case $(ARCH) in \
		armv8l) \
			$(CC) -s -static -o $@ $<; \
			;; \
